        ...
        else if (subFont.pointSize == minFontSize)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (i = [widths count] - 1; i != NSNotFound; i--)
                    {
                    ...
                    UIGraphicsPopContext();
                    if (actualFontSize) *actualFontSize = subFont.pointSize;
                    if (charactersFitted) *charactersFitted = MAX(0u, i);
...
{
    if(self.minSamples > samples){
        samples = self.minSamples;
    }
    
    if(self.maxSamples < samples){
